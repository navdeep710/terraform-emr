...
resource "aws_iam_role" "ec2_autoscaling" {
  ...
  assume_role_policy = join("", data.aws_iam_policy_document.assume_role_autoscaling.*.json)
}
...
//  vpc_id          = var.vpc_id
//  count           = var.enabled && var.subnet_type == "private" ? 1 : 0
//  service_name    = format("com.amazonaws.%s.glue", var.region)
//  auto_accept     = true
//  tags            = module.label.tags
//  security_group_ids = ["${aws_security_group.managed_master[count.index].id}"]
